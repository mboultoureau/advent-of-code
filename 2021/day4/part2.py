...
grids = []
victory_grids = [False for _ in range(nb_of_grid)]
...
for n in numbers:
    for grid in grids:
        ...
        if grid.check_victory():
            victory_grids[grid.id] = True
            # Check if all board is completed
            for i in range(nb_of_grid):
                if len(set(victory_grids)) == 1 and victory_grids[0] == True:
